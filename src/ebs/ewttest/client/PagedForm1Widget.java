...
import ebs.ewt.client.widgets.form.EWTCheckGroupField;
...
public class PagedForm1Widget extends EWTPage {
	private static PagedForm1Widget instance = null;
	...
	public void init() {
		...
		grid = new EWTPagedGrid<PersoneDTO>(getName(), 50,
				new EWTGridColumn[] {
						new EWTGridColumn("id", "ID", 0).setBooleans(true, false, false).setLabel(true),
						...
						new EWTGridColumn("levels", "Levels", 0, EWTBaseDataRenderImpl.MULTILINE, true)
								.setBooleans(true, false, false)
								.setEditField(new EWTCheckGroupField(Level, "levels", "Levels", false)),
				...
				new EWTFormSubmitter<PersoneDTO>() {
					...
					public void save(PersoneDTO personeDTO) {
						EWT.display(personeDTO.toString());
						getResetCallback("Data saved!", "Data is not saved!").onSuccess(true);
					}

					@Override
					public void add(PersoneDTO personeDTO) {
						EWT.display(personeDTO.toString());
						getResetCallback("Data added!", "Data is not saved!").onSuccess(false);
					}
				},
				this);
		grid.generateBar(true);
